	...
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<div class="mb-8">
			<div class="flex flex-col sm:flex-row sm:justify-between">
				<div class="mb-4 sm:mb-0">
					<h1 class="text-3xl font-bold text-gray-900 dark:text-white">{{ __('messages.carriers.title') }}</h1>
					<p class="mt-2 text-gray-600 dark:text-gray-400">{{ __('messages.carriers.subtitle') }}</p>
				</div>
				...
				<form method="GET" class="space-y-4">
					<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
						<div class="md:col-span-2">
							...
							<input class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white" type="text" name="filter[q]" value="{{ request('filter.q') }}" placeholder="{{ __('messages.carriers.filters_search_ph') }}">
						</div>
						<div>
							<label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('messages.carriers.filters_trashed') }}</label>
							<select class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white" name="filter[trashed]">
								<option value="">{{ __('messages.carriers.active') }}</option>
		...
		<div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
			<div class="text-sm text-gray-600 dark:text-gray-400">
				{{ __('messages.carriers.filters_showing') }} {{ $carriers->firstItem() ?? 0 }} to {{ $carriers->lastItem() ?? 0 }} of {{ $carriers->total() }} {{ __('messages.carriers.title') }}
			</div>
		</div>
				...
				<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 m-auto" style="width: 90%;">
					<thead class="bg-gray-50 dark:bg-gray-700">
						<tr>
							...
							<th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">{{ __('Created_at') }}</th>
							...
							<tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200">
								...
								<td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
									<div class="flex items-center justify-end space-x-2">
										<a class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300" href="{{ route('carriers.edit',$c) }}" style="padding-right: 10px;">{{ __('messages.common.edit') }}</a>
										@if($c->trashed())
							...
							<tr>
								<td colspan="3" class="px-6 py-12 text-center">
									<div class="text-gray-500 dark:text-gray-400">
										...
										<h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">{{ __('messages.carriers.empty') }}</h3>
										<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('messages.carriers.get_started') }}</p>
										<div class="mt-6">
											<a href="{{ route('carriers.create') }}" class="inline-flex items-center px-6 py-3 border border-transparent shadow-sm text-base font-medium rounded-lg bg-blue-600 hover:bg-blue-700 transition-colors duration-200">{{ __('messages.carriers.new') }}</a>
										</div>
									</div>
								</td>
							</tr>
